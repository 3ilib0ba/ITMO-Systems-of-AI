...
void invoke_function(const string &a, const string &b, search_function function) {
    if (function(a, b)) {
...
pair<const string, search_function> search_functions[] = {
        ...
        { "iterative deepening search", iterative_depth_first_search },
        { "bidirectional search", bidirectional_search },
        { "first best match search", best_first_search },
        { "method of minimizing the summary score", a_star_search }
};

int main() {
    // Сохраняем кодировку консоли
    system("chcp 1251");

    // Начальные данные
    load_cities("C:\\InnerDesktop\\ITMO-All\\Systems-of-AI\\Labs\\Lab-2-alg\\data.data");
    load_paths("C:\\InnerDesktop\\ITMO-All\\Systems-of-AI\\Labs\\Lab-2-alg\\straight_forward.data");

    // find a path from "С.Петербург" to "Житомир"
    for (int i = 0; i < 6; i++) {
        cout << search_functions[i].first << ":" << endl;
        invoke_function("Брест", "Казань", search_functions[i].second);
        cout << endl;
    }
    return 0;
}
